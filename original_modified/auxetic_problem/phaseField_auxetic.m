function phaseField_auxetic(h,nelx,nely,e,nu,lv,lag,kappa,eta,wcoef,PDECycle,timeStep,u_target_out)
...
for l = 1:20
    ...
    u = FEM(nelx,nely,nodes,e,nu,lv,phi,phimin,phimax); %Calculate displacement u by FEM.
...
function [u] = FEM(nelx,nely,nodes,e,nu,lv,phi,phimin,phimax)
...
gk(din,din) = gk(din,din) + 0.05;
gk(dout,dout) = gk(dout,dout) + 0.05;
fixed_y_dofs = [2 :2: (1 * (nelx+1)) * 2];   % y contraints at bottom line
fixed_x_dofs = [2*((nelx+1)*([1:nely+1]-1)+1)-1];  % x constraint on vertical line
fixedDofs = sort(horzcat(fixed_y_dofs, fixed_x_dofs));
allDofs = [1:ndofs];
...
din = 2*(nelx+1)-1;
dout = 2*((nelx+1) * (nely+1 - 1) + 1);
ndofs = 2*(nelx+1)*(nely+1); %global number of DOFs
dcom = zeros(ne,1);
